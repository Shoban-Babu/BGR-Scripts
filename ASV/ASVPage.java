...
public class ASVPage extends BasePage{
	...
	public void RescheduleBooking(UserProfile userProfile){
		browser.wait(getWaitTime());
			Report.updateTestLog("Book An Annual Service Link is present in the Application", "WARN");
			verifyAndClickWithXpath(pageProperties.getProperty("ASV.RescheduleAppoitment"), "RescheduleAppoitment Link");
			verifyAndClickWithXpath(pageProperties.getProperty("ASV.RescheduleAppoitment"), "Cancel Booking Link");
	...
	public void verifyChooseYourAppointmentPage(){
		...
		verifyAndInputByXpath(pageProperties.getProperty("ASV.telephonenumbernew"),"Telephone Number", "01965234175");
		}
		
	public void verifyAppointmentSlots(){
		browser.wait(5000);
		Report.updateTestLog("Appointments are displayed", "WARN");
		String Day1 = "Tue";
		String Day2 = "Wed";
		...
		//String appDay1=  browser.getTextByXpath(pageProperties.getProperty("FastTrack.errorMsg1"));
		
		verifyAndClickWithXpath(pageProperties.getProperty("ASV.selectAppoitment"), "Select this Appoitment");
		verifyAndInputByXpath(pageProperties.getProperty("ASV.telephonenumbernew"),"Telephone Number", "01965234175");
		verifyIsElementVisibleWithXpath(pageProperties.getProperty("ASVIBReview.Continue"), "Confirm and Book");
		
		}
	
public void navigateToAppointmentconfirmationPage(){
	Report.updateTestLog("Confirmation Page is displayed", "WARN");
		if(browser.isElementVisibleWithXpath(pageProperties.getProperty("ASV.GoToAccountSummaryPage"))){
			verifyAndClickWithXpath(pageProperties.getProperty("ASV.GoToAccountSummaryPage"), "Go to Account Summary ");
			Report.updateTestLog("Account summary button is visible", "PASS");
		}
		else{
			Report.updateTestLog("Account summary button is not visible", "FAIL");
		}
