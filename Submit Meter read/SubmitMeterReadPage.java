...
public class SubmitMeterReadPage extends BasePage {
	...
	public void confirmAccountSelection() {
		...
		if (browser.isElementVisibleWithXpath(pageProperties.getProperty("SubmitMeterRead.confirmAccountSelectionXpath"))) {
		browser.clickWithXpath(pageProperties.getProperty("SubmitMeterRead.confirmAccountSelectionXpath"));
		}
		/*
	...
	public void setImPlausbileReadingHigh_New(String accountNumber) {
		browser.wait(getWaitTime());
		NewNectar();
	...
	public void setPlausbileReadingMeterReadNew(String accountNumber) {
		browser.wait(getWaitTime());
		NewNectar();
		//verifyAndSelectCheckBoxByID(accountNumber, "Select CheckBox");
		//verifyAndClickWithXpath("//input[@value='"+accountNumber+"']", "Select CheckBox");
	...
	public void confirmAccountSelection_New() {
		
		if (browser.isElementVisibleWithXpath(pageProperties.getProperty("SubmitMeterRead.ConfirmSubmitbutton"))) {
		browser.clickWithXpath(pageProperties.getProperty("SubmitMeterRead.ConfirmSubmitbutton"));
		}
		browser.wait(getWaitTime());
		if (browser.isElementVisibleWithXpath(pageProperties.getProperty("SubmitMeterRead.NectarCardBanner"))) {
			Report.updateTestLog("Nectar Card Number Detais Banner is available", "PASS");
			}
		else
		{
			Report.updateTestLog("Nectar Card Number Detais Banner is not available", "Fail");
		}
		browser.wait(3000);
		if (browser.isElementVisibleWithXpath(pageProperties.getProperty("SubmitMeterRead.NectarPointsBanner"))) {
			Report.updateTestLog("Nectar Points Detais Banner is available", "Fail");
			}
		else
		{
			Report.updateTestLog("Nectar Points Banner is not available", "Pass");
		}
	}
	...
	public void NewNectar()
	{
		verifyAndClickWithXpath(pageProperties.getProperty("SubmitMeterRead.SelectAddNectar"),"Add my Nectar is selected");
		browser.wait(5000);
		verifyAndInputByXpath(pageProperties.getProperty("SubmitMeterRead.NectarNumber"), "Nectar Numbers", "11111111111");
		verifyAndClickWithXpath(pageProperties.getProperty("SubmitMeterRead.NectarTermsandConditions"),"select Terms and Condition Check box");
	...
	public void submitMeterReads() {
		...
		Report.updateTestLog("Your Meter Reading is Complete Page", "WARN");
	...
	public void navigateToSMRReadPage(){
		verifyIsTextPresent("Submit a meter reading");
		verifyAndClickWithXpath(pageProperties.getProperty("SubmitMeterRead.SubmitMeterReadinglink"), "Submit Meter Link"); 
	}
