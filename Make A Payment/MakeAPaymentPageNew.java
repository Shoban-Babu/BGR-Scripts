...
public class MakeAPaymentPageNew extends BasePage{
	 private final static String FILE_NAME = "resources/selfServe/MakePaymentCardDetailsPage.properties";
	    ...
	    
	    {
	    	...
	    	if(cardtype[card].equals("Visa Debit"))
	    	{
	    	verifyAndInputById(pageProperties.getProperty("MakePayment.CardNumber"), "Card number","4532957670106897");
	    	verifyAndInputById(pageProperties.getProperty("MakePayment.CVV"),"", "123");
	    	}
	    	if(cardtype[card].equals("Visa Delta"))
	    	{
	    	verifyAndInputById(pageProperties.getProperty("MakePayment.CardNumber"), "Card number","4539895801142071");
	    	verifyAndInputById(pageProperties.getProperty("MakePayment.CVV"),"", " 722");
	    	}
	    	if(cardtype[card].equals("Visa Credit Card"))
	    	{
	    	verifyAndInputById(pageProperties.getProperty("MakePayment.CardNumber"), "Card number","4485750574384323");
	    	verifyAndInputById(pageProperties.getProperty("MakePayment.CVV"),"", "152");
	    	}
	    	if(cardtype[card].equals("MasterCard"))
	    	{
	    	verifyAndInputById(pageProperties.getProperty("MakePayment.CardNumber"), "Card number","5535149700914409");
	    	...
	    	verifyAndInputById(pageProperties.getProperty("MakePayment.CVVNew"),"", "123");
	    ...
	    public void ClickMakeaPayment(){
	    	verifyAndClickWithXpath(pageProperties.getProperty("MakePayment.MakePaymentLinkNEW"), "Make a payment");
	    }
	    
	    public void EnterCVVNumber(){
	    	verifyAndInputByXpath(pageProperties.getProperty("MakePayment.Amounttopay"),"Amount To Pay","6");
	    	//verifyAndInputByXpath(pageProperties.getProperty("MakePayment.CVV"),"CVV","143");
	    	verifyAndInputById(pageProperties.getProperty("MakePayment.CVV"),"CVV", "123");
	    	verifyAndClickWithXpath(pageProperties.getProperty("MakePayment.Continue"), "Continue Button");
	    	Report.updateTestLog("Making Payment for Existing Card", "WARN");
	    }
...
public void makeAPaymentJourney_new(int card)
	    
	    {
           ...
           verifyAndInputByXpath(pageProperties.getProperty("MakePayment.CVV"),"CVV","123"); 	
	    	Report.updateTestLog("Payment Details Page", "WARN");
	    	verifyAndClickWithXpath(pageProperties.getProperty("MakePayment.Continue"), "MakeAPayment Continue");
	    	Report.updateTestLog("Payment Now Page", "WARN");
	    	} catch (Exception e) {
